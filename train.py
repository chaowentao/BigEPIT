        ...
        if (idx_epoch + 1) % step == 0 or idx_epoch > args.epoch - step:
            with torch.no_grad():
                ...
                for index, test_name in enumerate(test_Names):
                    ...
                    logger.log_string(
                        "The %dth Test on %s, psnr/ssim is %.2f/%.3f"
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, data_info, LF_name) in tqdm(
        ...
        def forward_chop(Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, net, crop_test_method=3):
            ...
            if args.crop_test_method == 3:
                ...
                Sr_SAI_y[:, :, 0:h_half, 0:w_half] = sr_list[0][
                    :, :, 0:h_half, 0:w_half
                ]
                Sr_SAI_y[:, :, 0:h_half, w_half:w] = sr_list[1][
                    :, :, 0:h_half, (w_size - w + w_half) : w_size
                ]
                Sr_SAI_y[:, :, h_half:h, 0:w_half] = sr_list[2][
                    :, :, (h_size - h + h_half) : h_size, 0:w_half
                ]
                Sr_SAI_y[:, :, h_half:h, w_half:w] = sr_list[3][
        ...
        if args.self_ensemble:
            ...
            sr_list = [
                forward_chop(
                    ...
                    Sr_SAI_cbcr,
                    net,
                    crop_test_method=args.crop_test_method,
                )
                for Lr_SAI_y in lr_list
            ]
            # print("len sr_list:", len(sr_list))
            # exit(0)
        ...
        if args.test_NTIRE2023_LFSR == 1:
            ...
            psnr, ssim = cal_metrics_2(
                Hr_SAI_y.squeeze(), Sr_SAI_y.squeeze(), args.angRes_out
            )
        ...
        if save_dir is not None:
            ...
            for i in range(args.angRes_out):
                for j in range(args.angRes_out):
                    img = Sr_4D_rgb[i, j, :, :, :]
                    path = (
                        str(views_dir) + "/View" + "_" + str(i) + "_" + str(j) + ".bmp"
                    )
                    imageio.imwrite(path, img)
                    pass
                pass
