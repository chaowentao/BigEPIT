...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        self.factor = self.scale = args.scale_factor
        ...
        self.upsampling = nn.Sequential(
            nn.Conv2d(
                channels, channels * self.scale**2, kernel_size=1, padding=0, bias=False
            ),
        ...
        self.upsample = nn.Sequential(
            nn.Conv2d(
                channels, channels * self.scale**2, kernel_size=1, stride=1, padding=0
            ),
            nn.PixelShuffle(self.scale),
    ...
    def forward(self, lr, info=None):
        ...
        x = rearrange(
            lr, "b c (u h) (v w) -> b c (u v) h w", u=self.angRes, v=self.angRes
        )
